...
@section('title')
  @parent - Modifier une personnalité
@endsection
      ...
      <div class="pl15 pr50">
       ...
       @endif
        ...
        <div class="center-block mw800">
          ...
          <form method="post" action="{{ route('celebrity.update', ['id' => $celebrity->id]) }}" id="form-ui">
          ...
          {{ csrf_field() }}
                ...
                <div class="row">
                  <div class="col-md-5">
                    <div class="section">
                      <label class="field">
                        <input type="text" name="firstname" id="firstname" class="gui-input" placeholder="Prénom" value="{{ $celebrity->firstname }}">
                      </label>
                    </div>
                  </div>
                  <div class="col-md-5">
                    <div class="section">
                      <label class="field">
                        <input type="text" name="lastname" id="lastname" class="gui-input" placeholder="Nom de famille" value="{{ $celebrity->lastname }}">
                      </label>
                    </div>
                  </div>
                  <div class="col-md-2">
                      ...
                      @foreach($pictures as $picture)
                        <option value="{{ $picture->id }}"
                          @if($celebrity->picture_id == $picture->id)
                            selected
                          @endif
                          data-thumbnail="{{ $picture->url }}">{{ $picture->alias }}</option>
                      @endforeach
                    </select>
                  </div>
                  <div class="col-md-4">
                    <div class="section">
                      <label class="field">
                        <input type="text" name="size" id="size" class="gui-input" placeholder="Taille (en cm)" value="{{ $celebrity->size }}">
                      </label>
                    </div>
                  </div>
                </div>
